package cn.tfl.bookstore.user.service;

import cn.tfl.bookstore.user.dao.UserDao;
...
public class UserService {
	...
	public void active(String code) throws UserException {
		 ...
		 */
		if(user.isState()) throw new UserException("用户已经激活，不需要重新激活");
		/*
		 * 4. 修改用户的状态
		 */
	...
	public User login(User form) throws UserException {
		 ...
		 * 5. 返回user
		 */
		User user=userDao.findByUsername(form.getUid());
		if(user==null) throw new UserException("用户不存在");
		if(!user.getPassword().equals(form.getPassword())){
			throw new UserException("密码错误");
		}
		if(!user.isState()) throw new UserException("尚未激活状态");
	    return   user；//千万不要反悔form
	}
